...
async function registerGopls(context, config) {
    const getGoplsPath = () => {
    ...
    const clientOptions = {
        documentSelector: ['go'],
        initializationOptions: () => coc_nvim_1.workspace.getConfiguration().get('go.goplsOptions', {})
    };
...
async function registerPlaygroud(context) {
    context.subscriptions.push(coc_nvim_1.commands.registerCommand("go.version", () => commands_1.version()), coc_nvim_1.commands.registerCommand("go.playground", async () => playground_1.openPlayground(await editor_1.activeTextDocument())));
}
